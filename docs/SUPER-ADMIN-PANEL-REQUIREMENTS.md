...
| FR-5.2a | The workspace is being replaced by the office web app's `/support` route (web requirements FR-15). Which one an operator gets is decided per server by `servers.app_url`: set, the console hands off to `${app_url}/support`; `NULL`, it hands off to `/workspace/` as before. Both stay deployed through the change | M | Setting or clearing the column moves a server's operators either way with no deploy; the choice is audited as `server_update` |
...
| FR-5.3a | From `/support` the web app cannot revoke anything — it is on another origin and holds no platform credentials — so its Exit returns to `/admin/#/?exit=<sid>` and the console revokes under the operator's own login. Best-effort: an unrevoked session still expires on its own hour | M | Revocation survives the console login having lapsed in the meantime: it runs when the operator signs back in, not only at page load |
